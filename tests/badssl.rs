...
async fn test_tls12_impl(vectored: bool) -> io::Result<()> {
    ...
    let mut config = rustls::ClientConfig::builder(Arc::new(provider))
        .with_root_certificates(root_store)
        .with_no_client_auth()
        .unwrap();
    // This interoperability endpoint does not negotiate RFC 7627 EMS.
    config.require_ems = false;
...
async fn test_modern_impl(vectored: bool) -> io::Result<()> {
    let mut root_store = rustls::RootCertStore::empty();
    root_store.extend(webpki_roots::TLS_SERVER_ROOTS.iter().cloned());
    let mut config = rustls::ClientConfig::builder(utils::provider())
        .with_root_certificates(root_store)
        .with_no_client_auth()
        .unwrap();
    // This interoperability endpoint does not negotiate RFC 7627 EMS.
